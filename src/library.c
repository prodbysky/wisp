...
Library prepare_library(const char* root_path) {
    ...
    for (size_t i = 0; i < lib.albums.count; i++) {
        ...
        qsort(lib.albums.items[i].tracks.items, lib.albums.items[i].tracks.count, sizeof(Track*), compare_track_by_number);
    }
    return lib;
}
...
static void filter_paths(Paths* paths, bool (*pred)(char*)) {
    for (size_t i = paths->count; i > 0; i--) if (!pred(paths->items[i - 1])) yar_remove(paths, i - 1, 1);
}
