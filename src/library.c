...
void mp3_processor(void* user_data, const drmp3_metadata* meta) {
    ...
    while (pos + 10 <= size) {
        ...
        uint32_t frame_size = (data[pos + 4] << 24) | (data[pos + 5] << 16) | (data[pos + 6] << 8) | (data[pos + 7]);
...
void unload_library(Library* lib) {
    for (size_t i = 0; i < lib->tracks.count; i++) {
        if (lib->tracks.items[i].album != NULL) free(lib->tracks.items[i].album);
        if (lib->tracks.items[i].artist != NULL) free(lib->tracks.items[i].artist);
        if (lib->tracks.items[i].title != NULL) free(lib->tracks.items[i].title);
    }
...
static Track process_track(const char* path) {
    ...
    if (strstr(path, ".flac")) {
        ...
        if (FLAC__metadata_get_picture(path, &picture, FLAC__STREAM_METADATA_PICTURE_TYPE_FRONT_COVER, NULL, NULL, -1,
                                       -1, -1, -1)) {
            int x, y, chan;
            uint8_t* image =
                stbi_load_from_memory(picture->data.picture.data, picture->data.picture.data_length, &x, &y, &chan, 3);
...
static bool filter(char* c) { return strstr(c, ".flac") != NULL || strstr(c, ".mp3") != NULL; }
static void filter_paths(Paths* paths, bool (*pred)(char*)) {
    for (size_t i = paths->count; i > 0; i--)
        if (!pred(paths->items[i - 1])) yar_remove(paths, i - 1, 1);
}
